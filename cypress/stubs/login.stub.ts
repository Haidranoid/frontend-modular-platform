...
export const loginSuccessfulStub = (email: string) => {
  return cy.fixture<Array<LoginRequestResponse>>('responses/login.success').then((loginResponsesFixtures) => {
    const loginResponse = loginResponsesFixtures.find((response) => response.user.email === email)
